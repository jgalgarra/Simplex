...
#twosteppred <- read.csv("../results/ALLFEATURES_pred.csv")
...
twosteppred$error <- abs(twosteppred$real - round(twosteppred$prediction))

rmse_abio <- rmse(abiopred$real,abiopred$prediction)
rse_abio <- rse(abiopred$real,abiopred$prediction)
abiopred$error <- abs(abiopred$real - round(abiopred$prediction))
...
plot_error <- function(datos,relleno="blue",titulo=""){
  p <- ggplot(data=datos)+
       ...
       ylim(c(0,300))+theme_bw()+ylab("Error")+xlab("Individuals")+
