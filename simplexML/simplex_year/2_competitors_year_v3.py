...
train_list = ['year','species', 'individuals', 
       'ph', 'salinity', 'cl', 'co3', 'c', 'mo', 'n', 'cn', 'p', 'ca', 'mg',
       'k', 'na', 'precip', 'present']
...
variables_to_ignore = ['year']
selected_features = [element for element in train_list if element not in variables_to_ignore]


individuals_model_train = conditions.loc[conditions['year'].isin([2015,2016,2017,2018])][selected_features]
individuals_model_test = conditions.loc[conditions['year'] == 2019][selected_features]
...
X_train_species = pd.DataFrame(data = individuals_model_train, columns = selected_features)
y_train_species = conditions.loc[conditions['year'].isin([2015,2016,2017,2018])][features_to_pred]

X_test_species = pd.DataFrame(data = individuals_model_test, columns = selected_features)
y_test_species = conditions.loc[conditions['year'] == 2019][features_to_pred]
